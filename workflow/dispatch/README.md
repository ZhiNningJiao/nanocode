...
## 可假设项 / 必须实测项 / 阻断项 / 非阻断后续
<!-- owner 2026-09-18 决策：业务可带显式假设并行推进，结果与视觉优先；
     验证按风险分层，只有本次真实阻断风险才卡发布，不机械每单双腿多轮。
     算法坐标/权重/数值回归仍属「必须实测项」。 -->
- 可假设项（写明假设内容，验证放行，结果反证即回滚）：<…>
- 必须实测项（本次发布前真跑）：算法/坐标/权重/数值回归、真实入口 E2E
- 阻断项（命中即卡发布，其余放行）：<本次真实阻断风险，写具体条件>
- 非阻断后续（不卡本单，登记 TODO/下一段续派）：<…>
