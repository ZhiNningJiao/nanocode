...
## [BUG][待执行] 手机打开对话调不出历史记录(2026-06-20 主人报)
电脑端发消息后,手机打开同一对话**看不到历史消息**(agent 回复不显示)。怀疑历史按目录/会话读取在移动端失效(跨实例/跨设备没拉到 jsonl 回放)。复现:电脑发消息→手机打开对话→历史空。影响:主人手机上看不到 agent 说了啥,信息断层。
