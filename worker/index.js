...
// Keep idle keep-alive sockets ALIVE on the worker side longer than the
// router's proxy http.Agent holds them on the client side (4s timeout in
// server/proxy.js). Otherwise the worker silently closes a socket the
// agent still believes is fresh; the next request from the router
// writes to a dead socket -> ECONNRESET -> the proxy surfaces
// "worker unavailable" as a 502 to the browser, especially noticeable
// after any pause >5s (the Node default keepAliveTimeout).
