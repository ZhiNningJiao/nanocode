...
export function createGitCompareRoutes(store) {
  ...
  router.get('/api/projects/:id/git/compare', async (req, res) => {
    ...
    if (!isValidRef(base) || !isValidRef(head)) {
      return res.status(400).json({ error: 'invalid base/head ref' })
    }
    if (base === head) {
      return res.json({ base, head, ahead: 0, behind: 0, files: [] })
    }
    try {
      await git(['rev-parse', '--verify', `${base}^{commit}`], cwd)
      await git(['rev-parse', '--verify', `${head}^{commit}`], cwd)
    } catch {
      return res.status(400).json({ error: 'invalid ref (not found in repo)' })
    }
    try {
      const [nameStatusOut, numstatOut, diffOut, aheadOut, behindOut] = await Promise.all([
        ...
        git(['rev-list', '--count', `${base}..${head}`], cwd),
        git(['rev-list', '--count', `${head}..${base}`], cwd),
      ])
      ...
      res.json({ base, head, ahead, behind, files })
    } catch (e) {
      res.status(500).json({ error: 'git diff failed', detail: e.message })
    }
  })
