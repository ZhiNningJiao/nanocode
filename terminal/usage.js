 ...
 */
export async function buildAigwSourceSummary({ now = Date.now(), fetchImpl } = {}) {
  const probe = await probeAigwSpend({ fetchImpl })
  const base = {
...
export async function buildUsageSummary({ home, store, now = Date.now(), fetchImpl } = {}) {
  ...
  // AIGW (LiteLLM) — admin billing probe (/key/info soft_budget etc.)
  sources.push(await buildAigwSourceSummary({ now, fetchImpl }))
